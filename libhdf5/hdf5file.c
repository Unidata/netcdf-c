...
/**
 * @file
 * @internal The HDF5 layer of the netCDF-4 file functions.
 *
 * @author Ed Hartnett
 */
...
read_hdf5_att(NC_GRP_INFO_T *grp, hid_t attid, NC_ATT_INFO_T *att)
{
   ...
   if ((retval = get_netcdf_type(grp->nc4_info, hdf5_att->native_hdf_typeid, &(att->nc_typeid))))
      BAIL(retval);
           ...
           NC_DIM_INFO_T **dim)
{
   ...
   if (H5DSget_scale_name(datasetid, dimscale_name_att, NC_MAX_NAME) >= 0)
   {
                   ...
                   strlen(DIM_WITHOUT_VARIABLE)))
      {
         ...
         hdf5_dim->hdf_dimscaleid = datasetid;
         H5Iinc_ref(datasetid);        /* Increment number of objects using ID */
      }
   }
             ...
             const H5G_stat_t *statbuf)
{
   ...
   if (NULL == dim || (dim && !((NC_HDF5_DIM_INFO_T *)(dim->format_dim_info))->hdf_dimscaleid))
