 ...
 */
int
create_nc_sample(int sample, int iosysid, int format, char *filename,
		 int my_rank, int *ncid)
{
   switch(sample)
   {
   case 0:
 ...
 * @author Ed Hartnett */
int
check_nc_sample(int sample, int iosysid, int format, char *filename,
		int my_rank, int *ncid)
{
   switch(sample)
   {
		   ...
		   int *ncidp)
{
   ...
   /* Create the file. */
   if ((ret = PIOc_createfile(iosysid, &ncid, &format, filename,
			      NC_CLOBBER)))
      return ret;
