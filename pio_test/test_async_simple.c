...
#define NUM_COMP_PROCS 1
   ...
   /* Initialize test. */
   if ((ret = pio_test_init2(argc, argv, &my_rank, &ntasks, TARGET_NTASKS,
   ...
   if (my_rank < TARGET_NTASKS)
   {
	 ...
	 ERR(ret);
      ...
      /* 			  num_procs, NULL, NULL, NULL, PIO_REARR_BOX, */
      /* 			  NULL) != PIO_EINVAL) */
      /* 	 ERR(ERR_WRONG); */
      /* if (PIOc_init_async(test_comm, NUM_IO_PROCS, NULL, COMPONENT_COUNT, */
      /* 			  num_procs, NULL, NULL, NULL, TEST_VAL_42, */
      /* 			  iosysid) != PIO_EINVAL) */
      /* 	 ERR(ERR_WRONG); */
      ...
      /* 	 ERR(ERR_WRONG); */
      /* if (PIOc_init_async(test_comm, NUM_IO_PROCS, NULL, COMPONENT_COUNT, */
      /* 			  NULL, NULL, NULL, NULL, PIO_REARR_BOX, */
      /* 			  iosysid) != PIO_EINVAL) */
      /* 	 ERR(ERR_WRONG); */

      /* Initialize the IO system. */
      if ((ret = PIOc_init_async(test_comm, NUM_IO_PROCS, io_proc_list,
				 COMPONENT_COUNT, num_procs, (int **)proc_list,
				 NULL, NULL, PIO_REARR_BOX, iosysid)))
	 ERR(ERR_INIT);
	 ...
	 for (int flv = 0; flv < 1; flv++)
	 {
	    ...
	    for (int sample = 0; sample < 1; sample++)
	    {
		  ...
		  return ret;
	       sprintf(filename, "%s_%s_%d_%d.nc", TEST_NAME, iotype_name,
		       sample, my_comp_idx);

	       /* Create sample file. */
	       if ((ret = create_nc_sample(sample, iosysid[my_comp_idx],
					   flavor[flv], filename, my_rank,
					   NULL)))
	       	  ERR(ret);
