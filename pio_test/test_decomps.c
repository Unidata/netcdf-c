   ...
   if (my_rank < TARGET_NTASKS)
   {
      ...
      for (int r = 0; r < NUM_REARRANGERS; r++)
      {
         ...
         for (int io_test = 0; io_test < num_iotests; io_test++)
         {
            ...
            if ((ret = PIOc_Init_Intracomm(test_comm, NUM_IO4, STRIDE1, BASE0,
                                           rearranger[r], &iosysid)))
               ERR(ret);

            /* Set the error handler. */
            if ((ret = PIOc_set_iosystem_error_handling(iosysid, PIO_BCAST_ERROR,
                                                        NULL)))
               ERR(ret);
            ...
            if ((ret = create_decomposition_2d(TARGET_NTASKS, my_rank, iosysid,
                                               dim_len_2d, &ioid, PIO_INT)))
               return ret;

            /* Test decomposition read/write. */
            if ((ret = test_decomp_read_write(iosysid, ioid, num_flavors, flavor,
                                              rearranger[r], my_rank, test_comm)))
               return ret;
