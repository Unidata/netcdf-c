		...
		if (more[0] != '#') {
			...
			*p = '\0';
			...
			if (strcmp(more, "USE_CACHE") == 0) {
			    ...
			    int stat = parseproxy(state,v);
			    if(stat != OC_NOERR) return stat;
			} else if(strcmp(more, "CREDENTIALS.USER") == 0) {
			    state->credentials.identity = strdup(v);
			    if (!state->credentials.identity) return OC_ENOMEM;
			    if (ocdebug > 0)
				oc_log(LOGNOTE,"CREDENTIALS.USER: %s", state->credentials.identity);
			} else if(strcmp(more, "CREDENTIALS.PASSWORD") == 0) {
			    state->credentials.password = strdup(v);
			    if (!state->credentials.password) return OC_ENOMEM;
			    if (ocdebug > 0)
				oc_log(LOGNOTE,"CREDENTIALS.PASSWORD: %s", state->credentials.password);

			} else if(strcmp(more, "CREDENTIALS.SSL.CERTIFICATE") == 0) {
			    state->credentials.ssl_certificate = strdup(v);
			    if (!state->credentials.ssl_certificate) return OC_ENOMEM;
			    if (ocdebug > 0)
				oc_log(LOGNOTE,"CREDENTIALS.SSL.CERTIFICATE: %s", state->credentials.ssl_certificate);
			} else if(strcmp(more, "CREDENTIALS.SSL.AUTH") == 0) {
			    state->credentials.ssl_authority = strdup(v);
			    if (!state->credentials.ssl_authority) return OC_ENOMEM;
			    if (ocdebug > 0)
				oc_log(LOGNOTE,"CREDENTIALS.SSL.AUTH: %s", state->credentials.ssl_authority);
			} else if(strcmp(more, "CREDENTIALS.CA.INFO") == 0) {
			    state->credentials.cainfo = strdup(v);
			    if (!state->credentials.cainfo) return OC_ENOMEM;
			    if (ocdebug > 0)
				oc_log(LOGNOTE,"CREDENTIALS.CA.INFO: %s", state->credentials.cainfo);
			} else if(strcmp(more, "CREDENTIALS.CA.PATH") == 0) {
			    state->credentials.capath = strdup(v);
			    if (!state->credentials.capath) return OC_ENOMEM;
			    if (ocdebug > 0)
				oc_log(LOGNOTE,"CREDENTIALS.CA.PATH: %s", state->credentials.capath);
			} else if(strcmp(more, "CREDENTIALS.COOKIEFILE") == 0) {
			    state->credentials.cookiefile = strdup(v);
			    if (!state->credentials.cookiefile) return OC_ENOMEM;
			    if (ocdebug > 0)
				oc_log(LOGNOTE,"CREDENTIALS.COOKIEFILE: %s", state->credentials.cookiefile);
			} else if(strcmp(more, "CREDENTIALS.COOKIEJAR") == 0) {
			    state->credentials.cookiejar = strdup(v);
			    if (!state->credentials.cookiejar) return OC_ENOMEM;
			    if (ocdebug > 0)
				oc_log(LOGNOTE,"CREDENTIALS.COOKIEJAR: %s", state->credentials.cookiejar);
...
parseproxy(OCstate* state, char* v)
{
    ...
    if (credentials_in_url(v)) {
        ...
        extract_credentials(v, &state->proxy.username, &state->proxy.password,
                            &result_url);
        v = result_url;
    }
