...
DOBUILD () {
    ...
    mkdir -p $BDIR
    cd $BDIR
    cmake $CURDIR -G"$CMAKEGEN" -DCPACK_PACKAGE_FILE_NAME=NetCDF-$NAME -DENABLE_TESTS=OFF -DBUILD_SHARED_LIBS=$SHARED -DENABLE_DAP=$DAP -DENABLE_NETCDF_4=$NC4 -DCMAKE_PREFIX_PATH=$RESOURCE_DIR
    CHECKERR
