...
int
nc_put_var1(int ncid, int varid, const size_t *coord, const void *op)
{
   return NC_put_var1(ncid, varid, coord, op, NC_NAT);
}
...
int
nc_put_var1_text(int ncid, int varid, const size_t *coord, const char *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid,varid,coord,(void*)op, NC_CHAR);
}

int
nc_put_var1_schar(int ncid, int varid, const size_t *coord, const signed char *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid,varid,coord,(void*)op, NC_BYTE);
}

int
nc_put_var1_uchar(int ncid, int varid, const size_t *coord, const unsigned char *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid,varid,coord,(void*)op, NC_UBYTE);
}

int
nc_put_var1_short(int ncid, int varid, const size_t *coord, const short *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid,varid,coord,(void*)op, NC_SHORT);
}

int
nc_put_var1_int(int ncid, int varid, const size_t *coord, const int *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid,varid,coord,(void*)op, NC_INT);
}

int
nc_put_var1_long(int ncid, int varid, const size_t *coord, const long *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid, varid, coord, (void*)op, longtype);
}

int
nc_put_var1_float(int ncid, int varid, const size_t *coord, const float *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid, varid, coord, (void*)op, NC_FLOAT);
}

int
nc_put_var1_double(int ncid, int varid, const size_t *coord, const double *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid,varid,coord,(void*)op, NC_DOUBLE);
}

int
nc_put_var1_ubyte(int ncid, int varid, const size_t *coord, const unsigned char *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
...
int
nc_put_var1_ushort(int ncid, int varid, const size_t *coord, const unsigned short *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid,varid,coord,(void*)op, NC_USHORT);
}

int
nc_put_var1_uint(int ncid, int varid, const size_t *coord, const unsigned int *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid,varid,coord,(void*)op, NC_UINT);
}

int
nc_put_var1_longlong(int ncid, int varid, const size_t *coord, const long long *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid,varid,coord,(void*)op, NC_INT64);
}

int
nc_put_var1_ulonglong(int ncid, int varid, const size_t *coord, const unsigned long long *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid,varid,coord,(void*)op, NC_UINT64);
}

#ifdef USE_NETCDF4
int
nc_put_var1_string(int ncid, int varid, const size_t *coord, const char* *op)
{
   NC* ncp;
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   INITCOORD1;
   return NC_put_var1(ncid, varid, coord, (void*)op, NC_STRING);
}
		 ...
		 const char *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,NC_CHAR);
}
		  ...
		  const signed char *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,NC_BYTE);
}
		  ...
		  const unsigned char *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,T_uchar);
}
		  ...
		  const short *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,NC_SHORT);
}
		...
		const int *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,NC_INT);
}
		 ...
		 const long *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,T_long);
}
		  ...
		  const float *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,T_float);
}
		   ...
		   const double *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,T_double);
}
		  ...
		  const unsigned char *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,T_ubyte);
}
		   ...
		   const unsigned short *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,T_ushort);
}
		 ...
		 const unsigned int *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,T_uint);
}
		     ...
		     const long long *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,T_longlong);
}
		      ...
		      const unsigned long long *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,NC_UINT64);
}
		   ...
		   const char* *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_varm(ncid,varid,startp,countp,stride,imapp,(void*)op,NC_STRING);
}
		 ...
		 const char *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,NC_CHAR);
}
		  ...
		  const signed char *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,NC_BYTE);
}
		  ...
		  const unsigned char *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,T_uchar);
}
		  ...
		  const short *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,NC_SHORT);
}
		...
		const int *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,NC_INT);
}
		 ...
		 const long *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,T_long);
}
		  ...
		  const float *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,T_float);
}
		   ...
		   const double *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,T_double);
}
		  ...
		  const unsigned char *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,T_ubyte);
}
		   ...
		   const unsigned short *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,T_ushort);
}
		 ...
		 const unsigned int *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,T_uint);
}
		     ...
		     const long long *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,T_longlong);
}
		      ...
		      const unsigned long long *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,NC_UINT64);
}
		   ...
		   const char* *op)
{
   ...
   int stat = NC_check_id(ncid, &ncp);
   if(stat != NC_NOERR) return stat;
   return NC_put_vars(ncid,varid,startp,countp,stride,(void*)op,NC_STRING);
}
