...
/**************************************************/
/**************************************************/
/* Load the metadata from store into memory */
...
ncz_read_atts(NC_FILE_INFO_T* file, NC_OBJ* container)
{
	...
	/* Watch for special cases: _FillValue, _ARRAY_DIMENSIONS (xarray) */
	...
	if((stat = ncz_ensure_fill_value((NC_VAR_INFO_T*)container))) goto done;
    }
...
define_vars(NC_FILE_INFO_T* file, NC_GRP_INFO_T* grp, NClist* varnames)
{
	...
	/* chunks */
	{
		...
		if(var->ndims != rank)
		    {stat = THROW(NC_ENCZARR); goto done;}
		if((var->chunksizes = malloc(sizeof(size_t)*(rank+zvar->scalar))) == NULL)
		    {stat = NC_ENOMEM; goto done;}
		if(zvar->scalar) {
		    chunks[0] = 1;
		} else {
