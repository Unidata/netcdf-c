 ...
 *
 *   2011-jun-21 - Print time strings for time var attributes, by Dave Allured.
 *   Modified from ncdump.c in netcdf-4.1.3, file date 2011-jun-10.
 *   Summary of changes:
 *   1.  Add include file "print_att_times.c".
 *   2.  Modify pr_att() to call new print function.
 *   3.  Change handling of line terminator strings in pr_att().
 *   4.  Add argument specp to pr_att() calls.
 *   5.  Add temporary global variable max_len_copy.
 *   6.  Add temporary forward reference for get_timeinfo().
 *   Some changes were done in a specific way to avoid impacts
 *   to existing make files and header files.
 *********************************************************************/
...
static int max_len_copy;	/* temporary global, send param to lput2 */

static void get_timeinfo(int ncid, int varid, ncvar_t *vp);
		/* temporary, create forward reference for print_att_times */
...
lput2(
    ...
    const boolean first_item,		/* identify first item in list */
    const boolean wrap			/* line wrap control: true=enable, */
...
/* (1) Single space or newline/indent sequence, as needed. */
    ...
    make_newline = (wrap && (first_item || linep > max_len_copy));
...
/* Print interpreted time strings for an attribute. */
/*--------------------------------------------------------------------------
 *
 * 2011-jun-21	print_att_times.c: New support function for ncdump.
 *		Added by Dave Allured, NOAA/PSD/CIRES.
 *		Preliminary version, used as an include to ncdump.c.
 *		This version supports only primitive attribute types;
 *		  do not call for user defined types.
 *
/*--------------------------------------------------------------------------*/

/* Print interpreted, human readable (ISO) time strings for an
 * attribute of a CF-like time variable.
 *
...
pr_att(
    ...
    int varid,
    const char *varname,
    int ia,				/* attribute index number */
    fspec_t* specp			/* formatting specs */
    )
{
...
/*       printf ("\t%s %s", type_name, var.name); */
	 ...
	 printf ("%s", id < var.ndims-1 ? ", " : ")");
	  ...
	  pr_att(ncid, kind, varid, var.name, ia, specp);
      }
#ifdef USE_NETCDF4
   ...
   for (ia = 0; ia < ngatts; ia++) { /* print ia-th global attribute */
       pr_att(ncid, kind, NC_GLOBAL, "", ia, specp);
   }
...
main(int argc, char *argv[])
{
    ...
    set_max_len(max_len);
    max_len_copy = max_len;	/* temporary, send param to lput2 */
