...
$Id: old_nc4attr.c,v 1.1 2010/05/25 17:54:25 dmh Exp $
*/
...
nc_inq_att(int ncid, int varid, const char *name, nc_type *xtypep, size_t *lenp)
{
   ...
   if (!nc->nc4_info)
      return RENAME3(inq_att)(nc->int_ncid, varid, name, xtypep, lenp);
...
nc_inq_attid(int ncid, int varid, const char *name, int *attnump)
{
   ...
   /* Handle netcdf-3 files. */
   if (!nc->nc4_info)
      return RENAME3(inq_attid)(nc->int_ncid, varid, name, attnump);
...
nc_inq_attname(int ncid, int varid, int attnum, char *name)
{
   ...
   /* Handle netcdf-3 files. */
   if (!nc->nc4_info)
      return RENAME3(inq_attname)(nc->int_ncid, varid, attnum, name);
   ...
   *equal = 0;
   ...
   else 
   {
	 ...
	 return NC_NOERR;
      ...
      /* Learn some stuff for further comparisons. */
	      ...
	      const char *newname)
{
   ...
   /* Handle netcdf-3 files. */
   if (!h5)
      return RENAME3(rename_att)(nc->int_ncid, varid, name, newname);
...
nc_del_att(int ncid, int varid, const char *name)
{
   ...
   /* Handle netcdf-3 files. */
   if (!h5)
      return RENAME3(del_att)(nc->int_ncid, varid, name);   
	       ...
	       const void *op)
{
   ...
   if (!nc->nc4_info)
   {
	 ...
	 mem_type = NC_BYTE;
      ...
      {
	 case NC_NAT:
	    return RENAME3(put_att)(nc->int_ncid, varid, name, file_type, 
			      len, op);
	 case NC_BYTE:
	    return RENAME3(put_att_schar)(nc->int_ncid, varid, name, 
				     file_type, len, op);
	 case NC_CHAR:
	    return RENAME3(put_att_text)(nc->int_ncid, varid, name, 
				    len, op);
	 case NC_SHORT:
	    return RENAME3(put_att_short)(nc->int_ncid, varid, name, 
				     file_type, len, op);
	 case NC_INT:
	    if (mem_type_is_long)
	       return RENAME3(put_att_long)(nc->int_ncid, varid, name, 
				       file_type, len, op);
	    else
	       return RENAME3(put_att_int)(nc->int_ncid, varid, name, 
				      file_type, len, op);
	 case NC_FLOAT:
	    return RENAME3(put_att_float)(nc->int_ncid, varid, name, 
				     file_type, len, op);
	 case NC_DOUBLE:
	    return RENAME3(put_att_double)(nc->int_ncid, varid, name, 
				      file_type, len, op);
	 default:
	    return NC_EBADTYPE;
      }
	       ...
	       int mem_type_is_long, void *ip)
{
   ...
   if (!nc->nc4_info)
   {
	 ...
	 mem_type = NC_BYTE;
      ...
      {
	 case NC_NAT:
	    return RENAME3(get_att)(nc->int_ncid, varid, name, ip);
	 case NC_BYTE:
	    return RENAME3(get_att_schar)(nc->int_ncid, varid, name, ip);
	 case NC_CHAR:
	    return RENAME3(get_att_text)(nc->int_ncid, varid, name, ip);
	 case NC_SHORT:
	    return RENAME3(get_att_short)(nc->int_ncid, varid, name, ip);
	 case NC_INT:
	    if (mem_type_is_long)
	       return RENAME3(get_att_long)(nc->int_ncid, varid, name, ip);
	    else
	       return RENAME3(get_att_int)(nc->int_ncid, varid, name, ip);
	 case NC_FLOAT:
	    return RENAME3(get_att_float)(nc->int_ncid, varid, name, ip);
	 case NC_DOUBLE:
	    return RENAME3(get_att_double)(nc->int_ncid, varid, name, ip);
	 default:
	    return NC_EBADTYPE;
      }
   }
