...
fprintf(stderr,"\n");
    ...
    ncstat = buildvaraprojection3(&drno->dap,varainfo,startp,countp,stridep,&varaprojection);
    if(ncstat != NC_NOERR) {THROWCHK(ncstat); goto fail;}
...
fprintf(stderr,"Reusing cached fetch constraint: %s\n",
	...
	nclistpush(vars,(ncelem)varainfo->target);
        constraint.projections = cloneprojections(drno->dap.oc.constraint.projections);
	    ...
	    */
	    ...
	    nclistpush(tmp,(ncelem)clone);
            ncstat = mergeprojections3(&drno->dap,constraint.projections,tmp);
...
fprintf(stderr,"vara merge: %s\n",
	dumpprojections(constraint.projections));
        ...
        restrictprojection34(&drno->dap,vars,constraint.projections);
        constraint.selections = cloneselections(drno->dap.oc.constraint.selections);
