...
#define TEST "test_quantize"
#define FILE_NAME "tmp_quantize"
#define NDIM1 1
...
main(int argc, char **argv)
{
    ...
    snprintf(file_name,sizeof(file_name),template,FILE_NAME);
#ifdef DEBUG
...
#define NDIM2 2
    ...
    printf("**** testing quantization handling of non-floats...");
    {
	    ...
	    char basename[4096];
            ...
            sprintf(basename, "tmp_bitgroom_type_%d", xtype[t]);
            sprintf(fname, template, basename);
#ifdef DEBUG
            fprintf(stderr,"fname=|%s|\n",fname);
#endif

            /* Create file. */
            if (nc_create(fname, NC_NETCDF4, &ncid)) ERR;
            ...
            /* Check file. */
            {
                if (nc_open(fname, NC_NETCDF4, &ncid)) ERR;
