...
static int NC3_def_var_compress(int,int,int,const char*,int*);
#endif /*USE_NETCDF4*/
...
NC3_inq_var_all(int ncid, int varid, char *name, nc_type *xtypep, 
               int *ndimsp, int *dimidsp, int *nattsp, 
               int *shufflep, char** algorithmp, int* params,
               int *fletcher32p, int *contiguousp, size_t *chunksizesp, 
               int *no_fill, void *fill_valuep, int *endiannessp)
{
...
static int
NC3_def_var_compress(int ncid, int varid ,int useshuffle, const char* algorithmp, int* params)
{
    return NC_ENOTNC4;
}
